            ...
            <AnimatePresence>
                {openCard && openIndex !== null && (
                    <>
                        ...
                        {/* 解説パネル */}
                        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8 md:px-8 md:py-16 pointer-events-none">
                            <motion.div
                                initial={{ opacity: 0, y: 24 }}
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0, y: 16 }}
                                transition={{ duration: 0.3, ease: "easeOut" }}
                                className="relative w-full max-w-6xl overflow-hidden pointer-events-auto grid grid-cols-1 md:grid-cols-2"
                            ...
                            >
                                ...
                                <div
                                    className="overflow-y-auto flex flex-col px-12 py-12 md:px-14 md:py-14"
                                    style={{ backgroundColor: openCard.color }}
                                >
                                    {/* タイトル（中央寄せ） */}
                                    <h2 className="font-bold text-3xl md:text-4xl text-[#1a1a1a] text-center leading-tight mb-3">
                                        {openCard.title}
                                    </h2>
                                    <p className="text-sm text-[#1a1a1a]/60 text-center mb-8">
                                        {openCard.subtitle}
                                    </p>
